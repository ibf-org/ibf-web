  ...
  return (
    <div className="mx-auto max-w-6xl p-6 lg:p-8">
      <div className="mb-8">
        ...
        <p className="text-sm text-gray-400">Track the status of your project applications</p>
      </div>

      {/* STATS ROW */}
      <div className="mb-8 grid grid-cols-2 gap-4 lg:grid-cols-4">
        <div className="rounded-xl border border-[#1e1e3a] bg-[#111827] p-4 text-center sm:text-left transition-all hover:border-teal-500/30">
          <div className="text-xs font-semibold uppercase tracking-wider text-gray-500 mb-1">Total Applied</div>
          <div className="text-3xl font-bold text-[#f0f0ff]">{stats.total}</div>
        </div>
        <div className="rounded-xl border border-[#1e1e3a] bg-[#111827] p-4 text-center sm:text-left transition-all hover:border-gray-400/30">
        ...
        <div className="rounded-xl border border-[#1e1e3a] bg-[#111827] p-4 text-center sm:text-left transition-all hover:border-blue-500/30">
          <div className="text-xs font-semibold uppercase tracking-wider text-blue-500/50 mb-1">Reviewing</div>
          <div className="text-3xl font-bold text-blue-400">{stats.reviewing}</div>
        </div>
        <div className="rounded-xl border border-[#1e1e3a] bg-[#111827] p-4 text-center sm:text-left transition-all hover:border-emerald-500/30">
      ...
      {/* TABLE / LIST */}
      <div className="overflow-hidden rounded-xl border border-[#1e1e3a] bg-[#111827]">
        {applications.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-20 px-4 text-center">
            <div className="mb-4 text-4xl opacity-50">📋</div>
            <h3 className="mb-2 text-lg font-bold text-[#f0f0ff]">You haven't applied to any projects yet.</h3>
            <p className="mb-6 text-sm text-gray-400">Discover exciting startups looking for your skills.</p>
            <Link 
              href="/discover"
              className="inline-flex items-center gap-2 rounded-lg bg-teal-500 px-6 py-2.5 text-sm font-bold text-white shadow-lg shadow-teal-500/20 transition hover:bg-teal-600"
            >
          ...
          <div className="w-full overflow-x-auto">
            <table className="w-full text-left text-sm text-gray-300">
              <thead className="bg-[#0C0F14]/50 text-xs uppercase text-gray-500 border-b border-[#1e1e3a]">
                <tr>
              ...
              <tbody className="divide-y divide-[#1e1e3a]">
                {applications.map((app) => (
                  <tr key={app.id} className="transition-colors hover:bg-[#1e1e3a]/30">
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <div className="text-[11px] text-gray-400">by {app.founderName}</div>
                        </div>
                      </div>
                    </td>
                    <td className="px-6 py-4">
                    ...
                    <td className="px-6 py-4">
                      {app.status === 'pending' && <span className="inline-flex items-center rounded-full bg-gray-500/10 px-2.5 py-1 text-xs font-medium text-gray-400 border border-gray-500/20">Pending review</span>}
                      {app.status === 'reviewing' && <span className="inline-flex items-center rounded-full bg-blue-500/10 px-2.5 py-1 text-xs font-medium text-blue-400 border border-blue-500/20">Reviewing</span>}
      ...
      {withdrawingId && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          <div className="absolute inset-0 bg-black/60 backdrop-blur-sm" onClick={() => !isDeleting && setWithdrawingId(null)} />
          <div className="relative w-full max-w-sm rounded-2xl border border-[#1e1e3a] bg-[#0C0F14] p-6 shadow-2xl animate-in zoom-in-95 duration-200 text-center">
            ...
            <div className="flex gap-3">
              <button 
                ...
                disabled={isDeleting}
                className="flex-1 rounded-xl bg-[#111827] py-2.5 text-sm font-semibold text-[#f0f0ff] hover:bg-[#1e1e3a] transition disabled:opacity-50"
              >
                Cancel
              </button>
              <button 
                ...
                disabled={isDeleting}
                className="flex-1 flex justify-center items-center rounded-xl bg-red-500 py-2.5 text-sm font-bold text-white shadow-lg shadow-red-500/20 hover:bg-red-600 transition disabled:opacity-50"
              >
