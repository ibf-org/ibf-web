    ...
    return (
      <div className="mb-6 border-b border-ibf-border pb-4 last:border-0 last:pb-0">
        <button onClick={() => setOpen(!open)} className="mb-3 flex w-full items-center justify-between font-bold text-ibf-heading">
          <span className="font-sans text-[12px] font-extrabold uppercase tracking-widest text-ibf-muted hover:text-ibf-heading transition-colors">{title}</span>
          ...
          <div className="custom-refinement-list text-[14px]">
            <RefinementList 
              ...
              classNames={{
                ...
                labelText: 'font-sans text-[14px] text-ibf-body group-hover:text-ibf-heading font-medium',
                count: 'ml-auto rounded-full bg-ibf-surface-2 px-2 py-[2px] font-sans text-[11px] font-bold text-ibf-muted'
              }}
            />
          </div>
        )}
        ...
        <div className="relative h-[150px] w-full shrink-0 bg-ibf-surface-2 overflow-hidden">
          {project.cover_url ? (
            ...
            <div className="flex h-full w-full items-center justify-center bg-gradient-to-br from-ibf-secondary-light to-ibf-primary-light hover-zoom-img">
              <span className="font-serif italic text-6xl text-ibf-heading opacity-20">{project.title.charAt(0)}</span>
            </div>
          )}
          {project.category && (
            <div className="absolute left-4 top-4 rounded-full bg-white/90 backdrop-blur-md px-3 py-1 font-sans text-[11px] font-extrabold uppercase tracking-wider text-ibf-heading shadow-sm">
              {project.category}
            </div>
          )}
        </div>
        ...
        <div className="flex flex-1 flex-col p-6">
          <div className="mb-3 flex items-center gap-2.5">
            <div className="flex h-[28px] w-[28px] shrink-0 items-center justify-center overflow-hidden rounded-full border border-ibf-border bg-white shadow-sm">
              {project.founder_avatar ? (
                ...
                <span className="font-sans text-[12px] font-bold text-ibf-heading">{project.founder_name?.charAt(0)}</span>
              )}
            </div>
            <div className="flex flex-col">
              <span className="font-sans text-[13px] font-bold text-ibf-heading leading-none">{project.founder_name}</span>
              <span className="font-sans text-[11px] font-medium text-ibf-muted mt-0.5">Founder</span>
            </div>
          </div>

          <h3 className="m-0 mb-1.5 font-sans text-[20px] font-extrabold tracking-tight text-ibf-heading line-clamp-1">{project.title}</h3>
          ...
          <div className="mt-auto">
            ...
            <div className="flex items-center justify-between border-t border-ibf-border pt-4">
              <div className="flex items-center gap-1.5 rounded-lg bg-ibf-surface-2 px-2.5 py-1.5 font-sans text-[12px] font-bold text-ibf-muted">
                <Briefcase size={14} />
                {openRolesCount} role{openRolesCount !== 1 ? 's' : ''}
              </div>
              <button 
                onClick={() => setSelectedProject(project)}
                className="btn-primary rounded-xl px-4 py-2 font-sans text-[13px] shadow-sm transform transition hover:-translate-y-0.5"
              >
                View Scope
              </button>
            </div>
    ...
    if (results && results.nbHits === 0) {
      return (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex w-full flex-col items-center justify-center py-32 text-center">
          ...
          <h3 className="mb-2 font-sans text-[24px] font-extrabold text-ibf-heading">No projects match</h3>
          <p className="font-sans text-[16px] font-light text-ibf-body max-w-sm">Try tweaking your search terms or loosening the filters to find what you're looking for.</p>
        </motion.div>
      )
    }
    return null
          ...
          <div className="relative mx-auto max-w-6xl">
            ...
            <SearchBox 
              ...
              classNames={{
                ...
                input: 'w-full rounded-full border-2 border-ibf-border bg-ibf-bg py-4 pl-14 pr-5 font-sans text-[16px] font-medium text-ibf-heading placeholder-ibf-muted transition-colors focus:border-ibf-secondary focus:outline-none focus:bg-white',
                submit: 'hidden',
                submitIcon: 'hidden',
        ...
        <div className="mx-auto flex max-w-7xl gap-10">
          ...
          <div className="hidden w-[260px] shrink-0 lg:block">
            <div className="sticky top-32 rounded-3xl border border-ibf-border bg-white p-6 shadow-sm">
              <div className="mb-8 flex items-center gap-2 font-sans text-[18px] font-extrabold text-ibf-heading tracking-tight">
      ...
      <AnimatePresence>
        {selectedProject && (
          <motion.div 
            ...
            className="fixed inset-y-0 right-0 z-[70] flex w-full max-w-[550px] flex-col border-l border-ibf-border bg-white shadow-2xl overflow-hidden font-sans"
          >
            <div className="flex shrink-0 items-center justify-between border-b border-ibf-border bg-ibf-bg/80 px-8 py-5 backdrop-blur-md">
              <span className="font-sans text-[12px] font-extrabold uppercase tracking-widest text-ibf-muted">Project Scope</span>
              <button 
                onClick={() => setSelectedProject(null)}
            ...
            <div className="flex-1 overflow-y-auto px-8 py-8">
              ...
              <div className="mb-6">
                <h2 className="mb-3 font-sans text-[36px] font-extrabold tracking-tight text-ibf-heading leading-none">
                  {selectedProject.title}
                </h2>
                <p className="font-serif italic text-[22px] text-ibf-muted leading-snug">
                  {selectedProject.tagline}
                </p>
              </div>
              
              <div className="mb-10 flex flex-wrap gap-2.5">
                <span className="inline-flex items-center gap-1.5 rounded-lg border border-ibf-border bg-ibf-surface px-3 py-1.5 font-sans text-[12px] font-bold text-ibf-heading">
                  <Building size={14} className="text-ibf-primary" /> {selectedProject.stage}
                </span>
                <span className="inline-flex items-center gap-2 rounded-lg border border-ibf-border bg-ibf-surface px-3 py-1.5 font-sans text-[12px] font-bold text-ibf-heading">
              ...
              <div className="mb-12">
                <h3 className="mb-4 font-sans text-[14px] font-extrabold uppercase tracking-widest text-ibf-heading border-b border-ibf-border pb-3">About the Vision</h3>
                <div 
              ...
              <div>
                <h3 className="mb-5 font-sans text-[14px] font-extrabold uppercase tracking-widest text-ibf-heading border-b border-ibf-border pb-3">Open Opportunities ({selectedProject.roles?.length || 0})</h3>
                ...
                {selectedProject.roles && selectedProject.roles.length > 0 ? (
                      ...
                      <div key={role.id} className="group rounded-[20px] border border-ibf-border bg-ibf-surface transition-all hover:shadow-md hover:border-ibf-border-2 overflow-hidden">
                        <details className="p-6">
                          <summary className="flex cursor-pointer select-none items-center justify-between outline-none marker:content-none">
                            <span className="flex items-center gap-3 font-sans text-[18px] font-extrabold text-ibf-heading">
                          ...
                          <div className="mt-6 border-t border-ibf-border pt-6 flex flex-col gap-6">
                            ...
                            <div className="grid grid-cols-2 gap-4 rounded-xl bg-white p-4 border border-ibf-border">
                              <div>
                                ...
                                <div className="font-sans text-[14px] font-bold text-ibf-heading">{role.commitment}</div>
                              </div>
                              <div>
                                <div className="text-[11px] font-bold uppercase text-ibf-muted mb-1">Compensation</div>
                                <div className="font-sans text-[14px] font-bold text-ibf-heading">{role.compensation}</div>
                              </div>
                            </div>
                            ...
                            <div>
                              ...
                              <div className="flex flex-wrap gap-2">
                                {role.skills_required?.map(skill => (
                                  <span key={skill} className="rounded-lg bg-white border border-ibf-border px-3 py-1.5 font-sans text-[12px] font-bold text-ibf-body shadow-sm">{skill}</span>
                                ))}
                              </div>
                            </div>
                  ...
                  <div className="rounded-2xl border border-ibf-border bg-ibf-surface p-6 text-center">
                    <p className="font-sans text-[14px] font-medium text-ibf-muted">No open roles currently available.</p>
                  </div>
                )}
              </div>
            </div>
            ...
            >
              <div className="rounded-[24px] bg-ibf-surface overflow-hidden">
                <div className="flex items-center justify-between border-b border-ibf-border px-8 py-6 bg-white">
                  <h3 className="font-sans text-[20px] font-extrabold text-ibf-heading flex items-center gap-2">
                ...
                <form onSubmit={submitApplication} className="p-8 bg-white pb-8">
                  <div className="mb-8 rounded-2xl border border-ibf-primary-mid/30 bg-ibf-primary-light/50 p-5 shadow-inner">
                    ...
                    <div className="font-sans text-[18px] font-extrabold text-ibf-heading">{applyingRole.title}</div>
                  </div>

                  <div className="mb-8">
                    <label htmlFor="coverNote" className="mb-3 block font-sans text-[14px] font-bold text-ibf-heading">
                    ...
                    <textarea 
                      ...
                      className="w-full resize-none rounded-2xl border-2 border-ibf-border bg-ibf-bg px-5 py-4 min-h-[160px] font-sans text-[15px] text-ibf-heading placeholder-ibf-muted transition-colors focus:border-ibf-secondary focus:bg-white focus:outline-none focus:shadow-sm"
                    />
                    <div className="mt-3 text-right font-sans text-[12px] font-medium text-ibf-muted">Max 500 characters</div>
                  </div>
                  ...
                  <div className="flex gap-4">
                    <button 
                      type="button" 
                      onClick={() => setApplyingRole(null)}
                      className="flex-1 rounded-xl bg-ibf-surface py-3.5 font-sans text-[14px] font-bold text-ibf-heading hover:bg-ibf-border transition-colors border border-ibf-border"
                    >
                      Cancel
                    </button>
                    <button 
                      type="submit"
                      className="btn-primary flex-[2] rounded-xl py-3.5 font-sans text-[14px] font-bold shadow-sm transform transition hover:-translate-y-0.5"
                    >
                      Confirm Application
                    </button>
                  </div>
